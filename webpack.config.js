...
module.exports = {
  ...
  plugins:[
    ...
    new WebpackPwaManifest({
      ...
      theme_color: "#ffffff",
      // Fingerprints tell webpack whether or not it should generate unique fingerprints so that 
      // each time a new manifest is generated, it looks like this: manifest.lhge325d.json. 
      fingerprints: false,
      // The inject property determines whether the link to the manifest.json is added to the HTML. 
      // Because we are not using fingerprints, we can also set inject to be false. We will hardcode 
      // the path to the manifest.json instead, just like we would in an application without webpack.
      inject: false,
      icons: [{
